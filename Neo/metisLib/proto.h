...
void Compute_Weights(GraphType *graph, idxtype *w); // changed by Joyce
...
//void One_level_overlap_refine(GraphType *graph, int nparts, idxtype *w, vector< list<int> >& AssVec, vector< list<int> >* new_AssVec); // added by Joyce
...
idxtype *idxmalloc_long(long long, char *); // Joyce added this
float *fmalloc(int, char *);
int *ismalloc(int, int, char *);
idxtype *idxsmalloc(int, idxtype, char *);
void *GKmalloc(int, char *);
void *GKmalloc_long(long long, char *); // Joyce added this
#endif
void GKfree(void **,...); 
